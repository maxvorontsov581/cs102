...
def group(values: List, n: int) -> List[List]:
    """Сгруппировать значения в списки по n элементов"""
...
def is_valid(grid: List[List[str]], pos: Tuple[int, int], num: str) -> bool:
    """Проверить, можно ли поставить число"""
    return not (num in get_row(grid, pos) or 
                num in get_col(grid, pos) or 
                num in get_block(grid, pos))
